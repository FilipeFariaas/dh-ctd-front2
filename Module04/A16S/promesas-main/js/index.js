...
function renderizarDadosUsuario(dados) {
    ...
    const {primeiro, ultimo} = dados.resultado[0].nome;
    const {rua, cidade, Estado} = dados.resultado[0].localizacao;
    const email = dados.resultado[0].email;
    const celular = dados.resultado[0].celular;
    const {grande} = dados.resultado[0].imagem;
    ...
    const template = `
        ...
        <div class="user-info">
            <h1 class="fullname" >${primeiro} ${ultimo}</h1>
            <p class="address" >${rua} - ${cidade} - ${Estado}</p>  
            <p class="email" >${email}</p>
            <p class="phone" >${celular}</p>
        </div>
    `;
